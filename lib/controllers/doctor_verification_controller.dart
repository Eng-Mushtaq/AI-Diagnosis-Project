...
class DoctorVerificationController extends GetxController {
  ...
  Future<void> getVerificationDocuments(String doctorId) async {
    ...
    try {
      final documents = await _supabaseService.getVerificationDocuments(doctorId);
  ...
  Future<bool> updateVerificationStatus(
    ...
    try {
      ...
      _errorMessage.value = 'Failed to update verification status: ${e.toString()}';
      return false;
    } finally {
      _isLoading.value = false;
    }
  ...
  Future<String> getVerificationStatus(String doctorId) async {
    try {
      final doctorData = await _supabaseService.supabaseClient
          .from('doctors')
